# Backup Alarm

**Equipment:** JLG G5-18A

1. Backup Alarm  The back-up alarm is located at the rear of the machine.When the park brake is released and the transmission shift control switch (transmission) is shifted to the (R) REVERSE position, the back-up alarm will automatically sound.  a.   Disassembly DO NOT   disassemble the back-up alarm. Replace a defective or faulty alarm with a new part.  b.   Inspection and Replacement  Inspect the wiring harness connector and alarm terminals for continuity and shorting. Test the alarm by turning the ignition key switch to the RUN position and shifting the transmission into the REVERSE position. The alarm should sound. Replace a defective or faulty alarm with a new part.  9.12.2   Ignition Key Switch a.   Ignition Switch Removal
2. Place a Do Not Operate Tag on both the ignition key switch and the steering wheel, stating that the machine should not be operated.
3. Open the battery and engine covers. Allow the engine to cool.
4. Install the lower dash panel.
5. Remove lower dash panel.
6. Remove the hex nut securing the ignition key switch to the dash.
7. Reach up under the dash to work the ignition switch and wiring out of the mounting hole.
8. Disconnect the ignition switch connectors from the cab harness connectors, and remove the switch from the machine.
